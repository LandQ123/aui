<template>
  <af-input
    class="af-date-editor"
    :class="'af-date-editor--' + type"
  ...
  <div
    class="af-date-editor af-range-editor af-input__inner"
    :class="[
      'af-date-editor--' + type,
    ...
    <div 
      ...
      class="af-input-group__prepend" 
      v-if="label">
      <span style="background:none;">{{ label }}</span>
    </div>
    ...
    <span class="af-range-separator">{{ rangeSeparator }}</span>
    <input
      autocomplete="off"
...
export default {
  ...
  computed: {
    ...
    triggerClass() {
      // return this.prefixIcon || (this.type.indexOf('date') < 0 ? 'af-icon-time' : 'af-icon-date');
      let regexp = /date|year|month|dates|week/g;

      return this.prefixIcon || (!regexp.test(this.type) ? 'af-icon-time' : 'af-icon-date');
    },
