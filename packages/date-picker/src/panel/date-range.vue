<template>
  <transition name="af-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      class="af-picker-panel af-date-range-picker af-popper"
      :class="[{
        'has-sidebar': $slots.sidebar || shortcuts,
        'has-time': showTime
      }, popperClass]">
      <div class="af-picker-panel__body-wrapper">
        <slot name="sidebar" class="af-picker-panel__sidebar"></slot>
        <div class="af-picker-panel__sidebar" v-if="shortcuts">
          <button
            type="button"
            class="af-picker-panel__shortcut"
            v-for="(shortcut, key) in shortcuts"
            :key="key"
            @click="handleShortcutClick(shortcut)">{{shortcut.text}}</button>
        </div>
        <div class="af-picker-panel__body">
          <div class="af-date-range-picker__time-header" v-if="showTime">
            <span class="af-date-range-picker__editors-wrap">
              <span class="af-date-range-picker__time-picker-wrap">
                <af-input
                  size="small"
                  :disabled="rangeState.selecting"
                  ref="minInput"
                  :placeholder="t('el.datepicker.startDate')"
                  class="af-date-range-picker__editor"
                  :value="minVisibleDate"
                  @input.native="handleDateInput($event, 'min')"
                  @change.native="handleDateChange($event, 'min')" />
              </span>
              <span class="af-date-range-picker__time-picker-wrap" v-clickoutside="handleMinTimeClose">
                <af-input
                  size="small"
                  :disabled="rangeState.selecting"
                ...
                <time-picker
                  ...
                  style="left:-100%;top: 32px;width: 250px;height:260px;box-shadow: none;border: none;">
                </time-picker>
              </span>
            </span>
            <span class="af-icon-arrow-right"></span>
            <span class="af-date-range-picker__editors-wrap is-right">
              <span class="af-date-range-picker__time-picker-wrap">
                <af-input
                  size="small"
                  :disabled="rangeState.selecting"
              ...
              <span class="af-date-range-picker__time-picker-wrap" v-clickoutside="handleMaxTimeClose">
                <af-input
                  size="small"
                  :disabled="rangeState.selecting"
                  ref="maxInput"
          ...
          <div class="af-picker-panel__content af-date-range-picker__content is-left">
            <div class="af-date-range-picker__header">
              <button
                type="button"
                @click="leftPrevYear"
                class="af-picker-panel__icon-btn af-icon-d-arrow-left"></button>
              <button
                type="button"
                @click="leftPrevMonth"
                class="af-picker-panel__icon-btn af-icon-arrow-left"></button>
              <button
                type="button"
                @click="leftNextYear"
                v-if="unlinkPanels"
                ...
                class="af-picker-panel__icon-btn af-icon-d-arrow-right"></button>
              <button
                type="button"
                @click="leftNextMonth"
                v-if="unlinkPanels"
                ...
                class="af-picker-panel__icon-btn af-icon-arrow-right"></button>
              <div>{{ leftLabel }}</div>
            </div>
            <date-table
          ...
          <div class="af-picker-panel__content af-date-range-picker__content is-right">
            <div class="af-date-range-picker__header">
              <button
                type="button"
                @click="rightPrevYear"
      ...
      <!-- <div class="af-picker-panel__footer" v-if="showTime">
        <af-button
          size="mini"
          type="text"
          class="af-picker-panel__link-btn"
          @click="handleClear">
  ...
  export default {
    ...
    methods: {
      ...
      handleRangePick(val, close = true) {
        ...
        if (!close || (this.$el.className.indexOf('has-time') >= 0 && !this.showTime)) {
          return;
        };
