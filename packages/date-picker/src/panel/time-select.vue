<template>
  <transition name="af-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('dodestroy')">
    <div
      ref="popper"
      v-show="visible"
      :style="{ width: width + 'px' }"
      :class="popperClass"
      class="af-picker-panel time-select af-popper">
      <af-scrollbar noresize wrap-class="af-picker-panel__content">
  ...
  export default {
    ...
    methods: {
      ...
      scrollToOption(selector = '.selected') {
        const menu = this.$refs.popper.querySelector('.af-picker-panel__content');
