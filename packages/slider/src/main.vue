<template>
  <div class="af-slider"
    :class="{ 'is-vertical': vertical, 'af-slider--with-input': showInput }"
     role="slider"
     :aria-valuemin="min"
     :aria-valuemax="max"
  ...
  >
    ...
    <div class="af-slider__runway"
      :class="{ 'show-input': showInput, 'disabled': sliderDisabled }"
      :style="runwayStyle"
      @click="onSliderClick"
      ref="slider">
      <div
        class="af-slider__bar"
        :style="barStyle">
      </div>
      <slider-button
        :vertical="vertical"
    ...
    methods: {
      ...
      setValues() {
        if (this.min > this.max) {
          console.error('[AUI Error][Slider]min should not be greater than max.');
          return;
        }
        ...
        if (this.range && Array.isArray(val)) {
          if (val[1] < this.min) {
            ...
            if (this.valueChanged()) {
              this.dispatch('AfFormItem', 'el.form.change', [this.minValue, this.maxValue]);
    ...
    computed: {
      stops() {
        ...
        if (this.step === 0) {
          ...
          console.warn('[AUI Warn][Slider]step should not be 0.');
          return [];
        }
