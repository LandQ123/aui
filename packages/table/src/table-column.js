...
import { getPropByPath } from 'aui/src/utils/util';
...
const defaults = {
  ...
  selection: {
    ...
    realWidth: 28,
    order: '',
    className: 'af-table-column--selection'
  },
  expand: {
    width: 28,
    minWidth: 28,
...
const forced = {
  ...
  expand: {
    ...
    renderCell: function(h, { row, store }, proxy) {
      ...
      return <div class={ 'af-table__expand-icon ' + (expanded ? 'af-table__expand-icon--expanded' : '') }
    ...
    sortable: false,
    resizable: false,
    className: 'af-table__expand-column'
  }
};
  ...
  created() {
      ...
      if (value !== undefined) {
        if (prop === 'renderHeader') {
          if (type === 'selection' && column[prop]) {
            console.warn('[AUI Warn][TableColumn]Selection column doesn\'t allow to set render-header function.');
          } else {
    ...
    column.renderCell = function(h, data) {
      ...
      return _self.showOverflowTooltip || _self.showTooltipWhenOverflow
        ? <div class="cell af-tooltip" style={ {width: (data.column.realWidth || data.column.width) - 1 + 'px'} }>{ renderCell(h, data) }</div>
