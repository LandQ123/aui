<template>
  ...
  >
    <div v-if="mode==='horizontal'" :class="`${prefix}-horizontal-wrapper`">
      <div :class="`${prefix}`">
        <slot></slot>
      </div>
    </div>
    <div v-else-if="mode==='vertical'" :class="`${prefix}-wrapper`" :style="wrapperStyle">
      <div :class="`${prefix}`">
        <div :class="[`${prefix}-ink`, `${prefix}-ink__${aside}`]">
          <span
            v-show="showInk"
...
<script>
import { scrollTop, findComponentsDownward, sharpMatcherRegx } from 'aui/src/utils/assist';
import { on, off } from 'aui/src/utils/dom';
export default {
  name: 'AfAnchor',
  componentName: 'AfAnchor',
  provide() {
    return {
      anchorCom: this
    };
  ...
  computed: {
    wrapperComponent() {
      return this.affix ? 'af-affix' : 'div';
    },
    wrapperStyle() {
      return {
  ...
  methods: {
    ...
    handleSetInkTop() {
      ...
      if (this.mode === 'vertical') {
        if (!this.links.includes(this.currentLink)) {
          this.inkTop = 0;
          return;
        }
    ...
    updateTitleOffset() {
      const links = findComponentsDownward(this, 'AfAnchorLink').map(link => {
        return link.href;
      });
