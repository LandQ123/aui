<script>
  ...
  import AfButtonGroup from 'aui/packages/button-group';
  import { generateId } from 'aui/src/utils/util';

  export default {
    name: 'AfDropdown',

    componentName: 'AfDropdown',
    ...
    watch: {
      visible(val) {
        this.broadcast('AfDropdownMenu', 'visible', val);
        this.$emit('visible-change', val);
      },
      focusing(val) {
        const selfDefine = this.$el.querySelector('.af-dropdown-selfdefine');
        if (selfDefine) { // 自定义
          if (val) {
    ...
    methods: {
      ...
      initAria() {
        ...
        if (!this.splitButton) { // 自定义
          ...
          this.triggerElm.setAttribute('class', (this.triggerElm.getAttribute('class') || '') + ' af-dropdown-selfdefine'); // 控制
        }
      },
      initEvent() {
    ...
    render(h) {
      ...
      let triggerElm = !splitButton
        ? this.$slots.default
        : (<af-button-group>
          <af-button type={type} size={dropdownSize} nativeOn-click={handleMainButtonClick}>
            {this.$slots.default}
          </af-button>
          <af-button ref="trigger" type={type} size={dropdownSize} class="af-dropdown__caret-button">
            <i class="af-dropdown__icon af-icon-arrow-down"></i>
          </af-button>
        </af-button-group>);

      return (
        <div class="af-dropdown" v-clickoutside={hide}>
          {triggerElm}
          {this.$slots.dropdown}
        </div>
      );
