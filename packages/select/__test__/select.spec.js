import { createTest, createVue, triggerEvent, destroyVM } from '../../../test/unit/util';
...
describe('Select', () => {
  ...
  it('create', () => {
    ...
    expect(vm.$el.className).to.equal('af-select');
    expect(vm.$el.querySelector('.af-input__inner').placeholder).to.equal('请选择');
    vm.toggleMenu();
  ...
  it('options rendered correctly', () => {
    vm = getSelectVm();
    const options = vm.$el.querySelectorAll('.af-select-dropdown__item');
  ...
  it('custom dropdown class', () => {
    ...
    const dropdown = vm.$el.querySelector('.af-select-dropdown');
  ...
  it('default value', done => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.af-input__inner').value).to.equal('双皮奶');
      done();
    }, 100);
  });
  ...
  it('single select', done => {
    vm = createVue({
      template: `
        <div>
          <af-select v-model="value" @change="handleChange">
            <af-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('disabled select', () => {
    ...
    expect(vm.$el.querySelector('.af-input').classList.contains('is-disabled')).to.true;
  });
  ...
  it('visible event', done => {
    vm = createVue({
      template: `
        <div>
          <af-select v-model="value" @visible-change="handleVisibleChange">
            <af-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('clearable', done => {
    ...
    setTimeout(() => {
      const iconClear = vm.$el.querySelector('.af-input__icon.af-icon-circle-close');
  ...
  it('object typed value', done => {
    vm = createVue({
      template: `
        <div>
          <af-select v-model="value" value-key="id">
            <af-option
              v-for="item in options"
              :label="item.label"
              :key="item.id"
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.af-input__inner').value).to.equal('label1');
      expect(vm.$el.querySelector('.af-select-dropdown__item').classList.contains('selected'));
      done();
    }, 100);
  });
  ...
  it('prefixed icon', () => {
      ...
      template: `
        <div>
          <af-select v-model="value">
            ...
            <i slot="prefix" class="af-input__icon af-icon-search"></i>
          </af-select>
        </div>
      `,
    ...
    expect(vm.$el.querySelector('.af-input__icon').classList.contains('af-icon-search')).to.be.true;
  });

  it('custom af-option template', () => {
    vm = createVue({
      template: `
        <div>
          <af-select v-model="value">
    ...
    expect(vm.$el.querySelector('.af-select-dropdown__item p').textContent).to.equal('label value');
  });
  ...
  it('option group', () => {
    ...
    const groups = vm.$el.querySelectorAll('.af-select-group__wrap');
    const options = groups[1].querySelectorAll('.af-select-dropdown__item');
  ...
  it('allow create', done => {
    ...
    setTimeout(() => {
      ...
      select.onInputChange();
      setTimeout(() => {
        const options = document.querySelectorAll('.af-select-dropdown__item span');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const tagCloseIcons = vm.$el.querySelectorAll('.af-tag__close');
  ...
  it('multiple remove-tag', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <af-select v-model="value" multiple @remove-tag="handleRemoveTag">
            <af-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('event:focus & blur', done => {
    vm = createVue({
      template: `
        <af-select ref="select"></af-select>
      `
    }, true);
  ...
  it('should return focus to input inside select after option select', done => {
    vm = createVue({
      template: `
        <div>
          <af-select v-model="value" ref="select">
            <af-option label="1" :value="1" />
          </af-select>
        </div>
      `,
      data() {
    ...
    const option = vm.$el.querySelectorAll('.af-select-dropdown__item')[0];
  ...
  it('only emit change on user input', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <af-select v-model="value" @change="change" ref="select">
            <af-option label="1" :value="1" />
            <af-option label="2" :value="2" />
            <af-option label="3" :value="3" />
          </af-select>
        </div>
      `,
      data() {
