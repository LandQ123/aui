    ...
    >
      ...
      <template slot="suffix">
        <i
          ...
          class="af-input__icon af-icon-circle-close af-cascader__clearIcon"
          @click="clearValue"
        ></i>
        <i
          key="2"
          v-else
          class="af-input__icon af-icon-caret-bottom"
...
import { generateId, escapeRegexpString, isIE, isEdge } from 'aui/src/utils/util';
...
export default {
  ...
  computed: {
    ...
    _afFormItemSize() {
      return (this.afFormItem || {}).afFormItemSize;
    },
    cascaderSize() {
      return this.size || this._afFormItemSize || (this.$ELEMENT || {}).size;
    },
    cascaderDisabled() {
  ...
  methods: {
    initMenu() {
      this.menu = new Vue(AfCascaderMenu).$mount();
    ...
    handleKeydown(e) {
      ...
      if (keyCode === 13) {
        ...
        setTimeout(() => {
          const firstMenu = this.popperElm.querySelectorAll('.af-cascader-menu')[0];
    ...
    highlightKeyword(label, keyword) {
      ...
      return label.split(keyword)
        .map((node, index) => index === 0 ? node : [
          h('span', { class: { 'af-cascader-menu__item__keyword': true }}, [this._v(keyword)]),
          node
        ]);
    },
