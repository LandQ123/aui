...
describe('Switch', () => {
  ...
  it('create', () => {
    vm = createTest(Switch, {
      ...
      inactiveText: 'off',
      activeInnerText: 'yes',
      inactiveInnerText: 'no',
      activeColor: '#0f0',
      inactiveColor: '#f00',
      width: 100
    });

    const core = vm.$el.querySelector('.af-switch__core');
    expect(core.style.backgroundColor).to.equal('rgb(255, 0, 0)');
    expect(core.style.width).to.equal('100px');
    expect(vm.$el.querySelector('.af-switch__label--left').querySelector('span').textContent).to.equal('off');
    expect(vm.$el.querySelector('.af-switch__core').querySelector('.text.is-active').textContent.trim()).to.equal('yes');
  });

  it('switch with icons', () => {
    vm = createTest(Switch, {
      activeIconClass: 'af-icon-check',
      inactiveIconClass: 'af-icon-close'
    });

    const icon = vm.$el.querySelector('.af-switch__label--left').querySelector('i');
  ...
  it('disabled switch should not respond to user click', done => {
    vm = createVue({
      template: `
        <div>
          <af-switch disabled v-model="value"></af-switch>
        </div>
      `,
    ...
    vm.$el.querySelector('.af-switch__core').click();
    Vue.nextTick(() => {
  ...
  it('expand switch value', done => {
    vm = createVue({
      template: `
        <div>
          <af-switch v-model="value" :active-value="onValue" :inactive-value="offValue"></af-switch>
        </div>
      `,
      data() {
        return {
  ...
  it('value is the single source of truth', done => {
    vm = createVue({
      template: `
        <div>
          <af-switch :value="true"></af-switch>
        </div>
      `
    }, true);
  ...
  it('sets checkbox value', done => {
    vm = createVue({
      template: `
        <div>
          <af-switch v-model="value"></af-switch>
        </div>
      `,
      data() {
        return {
