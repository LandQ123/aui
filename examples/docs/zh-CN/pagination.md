...
:::demo `total`表示总条目数，`size`用于设置每页显示的页码数量。
```html
<div class="block split-line">
...
:::demo 默认情况下，当总页数超过 7 页时，Pagination 会折叠多余的页码按钮。通过`pager-count`属性可以设置最大页码按钮数。设置`layout`，表示需要显示的内容，用逗号分隔，布局元素会依次显示。`prev`表示上一页，`next`为下一页，`start`表示第一页，`end`为最后一页，`pager`表示页码列表，除此以外还提供了`jumper`和`total`，`size`和特殊的布局符号`->`，`->`后的元素会靠右显示，`jumper`表示跳页元素
```html
<af-pagination
  :page-size="20"
  :pager-count="11"
...
:::
<script>
  import { addClass } from 'aui/src/utils/dom';
  export default {
    methods: {
      handleSizeChange(val) {
        console.log(`每页 ${val} 条`);
