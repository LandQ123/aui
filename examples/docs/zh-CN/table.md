...
:::demo 当`af-table`元素中注入`data`对象数组后，在`af-table-column`中用`prop`属性来对应对象中的键名即可填入数据，用`label`属性来定义表格的列名。可以使用`width`属性来定义列宽。
```html
  <template>
    <af-table
      :data="tableData"
...
使用不带斑马纹的表格，可以更容易区分出不同行的数据。

:::demo `stripe`属性可以创建带斑马纹的表格。它接受一个`Boolean`，默认为`true`，设置为`false`即为禁用。
```html
<template>
  <af-table
    :data="tableData"
...
:::demo 只要在`af-table`元素中定义了`height`属性，即可实现固定表头的表格，而不需要额外的代码。
```html
<template>
  <af-table
    :data="tableData3"
  ...
  <af-table
    ...
    <af-table-column
      ...
      <template slot-scope="scope">
        <af-button @click="handleClick(scope.row)" type="text" size="small">查看</af-button>
        <af-button type="text" size="small">编辑</af-button>
      </template>
    </af-table-column>
  </af-table>
</template>
...
:::demo 只需要在 af-table-column 里面嵌套 af-table-column，就可以实现多级表头。
```html
<template>
  <af-table
    :data="tableData3"
    ...
    <af-table-column header-align="center" label="配送信息">
      <af-table-column
        prop="name"
        label="姓名"
        width="120">
      </af-table-column>
      <af-table-column label="地址">
        <af-table-column
          prop="province"
          label="省份"
          width="120">
...
:::demo Table 组件提供了单选的支持，只需要配置`highlight-current-row`属性即可实现单选。之后由`current-change`事件来管理选中时触发的事件，它会传入`currentRow`，`oldCurrentRow`。如果需要显示索引，可以增加一列`af-table-column`，设置`type`属性为`index`即可显示从 1 开始的索引号。
```html
<template>
  <af-table
    ref="singleTable"
  ...
  <div style="margin-top: 20px">
    <af-button @click="setCurrent(tableData[1])">选中第二行</af-button>
    <af-button @click="setCurrent()">取消选择</af-button>
  </div>
</template>
...
:::demo 实现多选非常简单: 手动添加一个`af-table-column`，设`type`属性为`selection`即可；默认情况下若内容过多会折行显示，若需要单行显示可以使用`show-overflow-tooltip`属性，它接受一个`Boolean`，为`true`时多余的内容会在 hover 时以 tooltip 的形式显示出来。
```html
<template>
  <af-table
    ref="multipleTable"
  ...
  <div style="margin-top: 20px">
    <af-button @click="toggleSelection([tableData3[1], tableData3[2]])">切换第二、第三行的选中状态</af-button>
    <af-button @click="toggleSelection()">取消选择</af-button>
  </div>
</template>
    ...
    <af-table-column label="操作">
      <template slot-scope="scope">
        <af-button
          size="mini"
          type="text"
          @click="handleEdit(scope.$index, scope.row)">编辑</af-button>
        <af-button
          size="mini"
          type="text"
          @click="handleDelete(scope.$index, scope.row)">删除</af-button>
      </template>
    </af-table-column>
  </af-table>
</template>
...
:::demo 通过设置 type="expand" 和 `Scoped slot` 可以开启展开行功能，`af-table-column` 的模板会被渲染成为展开行的内容，展开行可访问的属性与使用自定义列模板时的 `Scoped slot` 相同。
```html
<template>
  <af-table
    :data="tableData5"
    ...
    <af-table-column type="expand">
      <template slot-scope="props">
        <!-- <af-form label-position="left" inline class="demo-table-expand">
          <af-form-item label="商品名称">
            <span>{{ props.row.name }}</span>
          </af-form-item>
          <af-form-item label="所属店铺">
            <span>{{ props.row.shop }}</span>
          </af-form-item>
          <af-form-item label="商品 ID">
            <span>{{ props.row.id }}</span>
          </af-form-item>
          <af-form-item label="店铺 ID">
            <span>{{ props.row.shopId }}</span>
          </af-form-item>
          <af-form-item label="商品分类">
            <span>{{ props.row.category }}</span>
          </af-form-item>
          <af-form-item label="店铺地址">
            <span>{{ props.row.address }}</span>
          </af-form-item>
          <af-form-item label="商品描述">
...
<style>
  ...
  .demo-table-expand .af-form-item {
    margin-right: 0;
    margin-bottom: 0;
    width: 50%;
  }
...
| resizable | 对应列是否可以通过拖动改变宽度（需要在 af-table 上设置 border 属性为真） | boolean | — | true |
